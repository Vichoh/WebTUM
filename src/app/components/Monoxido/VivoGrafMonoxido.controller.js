(function () {
  ...
  vivoGrafMonoxidoCtrl.$inject = ['UltimoIngresadoService', '$interval', '$scope', '$log'];

  function vivoGrafMonoxidoCtrl(UltimoIngresadoService, $interval, $scope, $log) {
    var vm = this;
    vm.location = 'line';
    var contMonoxido = 0;
    var id = -1;
    ...
    vm.posts = function () {
      UltimoIngresadoService.get().$promise.then(function (data) {
        ...
        if (data.id !== id) {
          ...
          id = data.id;
          contMonoxido++;
          $log.debug(contMonoxido);
          $log.debug(vm.monoxidos);
        }
      });
    };
